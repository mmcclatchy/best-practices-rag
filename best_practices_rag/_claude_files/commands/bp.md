...
- `OUTPUT_SLUG` = sorted tech names joined by `-` + `-` + topic keywords joined by `-`, truncated to 60 characters, then append `-codegen` (e.g., `fastapi-sqlalchemy-async-session-management-codegen`)
