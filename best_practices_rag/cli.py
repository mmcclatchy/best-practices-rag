...
from best_practices_rag.commands import codex_model, opencode_model
...
from best_practices_rag.tui_install import (
    _read_manifest,
...
models_app = typer.Typer(
    help="Configure model tier mappings for supported TUIs.",
    no_args_is_help=True,
)
...
@models_app.command(name="opencode", help="Sync OpenCode reasoning/task model tiers")
def _models_opencode(
...
@models_app.command(name="codex", help="Sync Codex reasoning/task model tiers")
def _models_codex(
    aa_key: str | None = typer.Option(
        None,
        "--aa-key",
    ...
    raise typer.Exit(
        codex_model.run(
            ...
            no_update_codex=no_update_codex,
            reasoning_model=reasoning_model,
            task_model=task_model,
            no_apply=no_apply,
        )
