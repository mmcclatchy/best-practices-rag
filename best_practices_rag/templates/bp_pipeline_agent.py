...
def generate_bp_pipeline_agent(adapter: TuiAdapter) -> str:
...
Always read `{synthesis_research}` regardless of MODE.
...
- If `MODE` is `codegen`, read `{synthesis_codegen}`
- If `MODE` is `research`, read `{synthesis_research}`
