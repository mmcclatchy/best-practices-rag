...
def get_adapter(kind: TuiKind) -> TuiAdapter:
    """Return the TUI adapter for the given kind."""
...
def detect_tuis() -> list[TuiKind]:
    """Detect which TUIs are installed on the system."""
    result: list[TuiKind] = []
    if shutil.which('claude'):
        result.append(TuiKind.CLAUDE)
    if shutil.which('opencode') or (
...
def parse_claude_agent(text: str) -> AgentSpec:
    """Parse a Claude Code YAML-frontmatter agent file into an AgentSpec."""
